...
const SearchTrip = () => {
  ...
  const filterTripsBySearch = async () => {
    ...
    const searchedTrips = allTrips.filter((trip) => {
      if(trip.title.toLowerCase().includes(searchedTrip.toLowerCase()) || trip.address.toLowerCase().includes(searchedTrip.toLowerCase())){
        return trip;
      }
    });
  ...
  return (
    <div className="flex justify-center mt-4 mb-4">
      <div className="flex gap-2">
        ...
        />
        <button
          className="bg-primary rounded-full text-white p-3"
          onClick={filterTripsBySearch}
        >
          <svg
